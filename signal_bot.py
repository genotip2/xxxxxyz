...
def check_btc_dominance():
    ...
    try:
        ...
        if last_btc_d is not None:
            ...
            if change > 0.3: status = "UPTREND"
            elif change < -0.3: status = "DOWNTREND"
            else: status = "NEUTRAL"
        else:
            status = "NEUTRAL"
...
def check_btc_condition():
    ...
    if is_uptrend and is_macd_bull and is_close_above: status = "BULLISH"
    elif is_uptrend: status = "SIDEWAYS"
    else: status = "BEARISH"
...
def calculate_entry_score(data_1d, data_4h, data_1h, current_price, sl_price):
    ...
    if data_1d['ema20'] > data_1d['ema50'] > data_1d['ema200'] and data_1d['close'] > data_1d['ema20']:
        score += 25; reasons.append("✅ 1D Strong Trend (EMA20>50>200) [+25]")
    elif data_1d['ema50'] > data_1d['ema200'] and data_1d['close'] > data_1d['ema50']:
        score += 20; reasons.append("✅ 1D Uptrend (Close>EMA50>200) [+20]")
    else:
        reasons.append("❌ 1D Trend Lemah [+0]")

    if data_1d['adx'] > 25: score += 15; reasons.append(f"✅ 1D ADX Kuat ({data_1d['adx']:.1f}) [+15]")
    else: reasons.append(f"❌ 1D ADX Lemah ({data_1d['adx']:.1f}) [+0]")

    # 2. PULLBACK (15%)
    if data_4h['ema20'] > data_4h['ema50']:
        dist_4h = abs(current_price - data_4h['ema20']) / data_4h['ema20'] * 100
        if dist_4h <= 2.0: score += 10; reasons.append(f"✅ 4H Perfect Pullback (Dist {dist_4h:.1f}%) [+10]")
        else: score += 5; reasons.append(f"⚠️ 4H Pullback Far (Dist {dist_4h:.1f}%) [+5]")
    else:
        reasons.append("❌ 4H Bukan Pullback [+0]")

    if 45 <= data_4h['rsi'] <= 60: score += 5; reasons.append(f"✅ 4H RSI Rebound ({data_4h['rsi']:.1f}) [+5]")
    else: reasons.append(f"⚠️ 4H RSI Tidak Ideal ({data_4h['rsi']:.1f}) [+0]")
    ...
    if data_1h['ema10'] > data_1h['ema20']: score += 5; reasons.append("✅ 1H Momentum (EMA10>20) [+5]")
    ...
    if 50 <= data_1h['rsi'] <= 65: score += 5; reasons.append(f"✅ 1H RSI Optimal ({data_1h['rsi']:.1f}) [+5]")
    else: reasons.append(f"⚠️ 1H RSI Tidak Optimal ({data_1h['rsi']:.1f}) [+0]")

    # 4. VOLUME (15%)
    vol, avg_vol = data_1h.get('volume', 0), data_1h.get('average_volume', 0)
    if avg_vol > 0 and vol > (1.5 * avg_vol): score += 15; reasons.append(f"✅ 1H Volume Spike ({vol/avg_vol:.1f}x) [+15]")
    else: reasons.append("❌ 1H Volume Rendah/Tidak Spike [+0]")
...
def check_entry(pair, data_1d, data_4h, data_1h, current_price, sl_price, btc_condition, btc_d_status):
    ...
    if not is_btc:
        if btc_condition == "BULLISH":
            score += 5; reasons.append("✅ BTC Bullish [+5]")
        elif btc_condition == "BEARISH":
            score -= 10; reasons.append("⚠️ BTC Bearish [-10]")
            
        if btc_d_status == "UPTREND":
            score -= 10; reasons.append("⚠️ BTC.D Uptrend [-10]")
        elif btc_d_status == "DOWNTREND":
            score += 5; reasons.append("✅ BTC.D Downtrend [+5]")
            
    score = max(0, min(100, score))
    
    buy_threshold = SCORE_BUY
    if not is_btc:
        if btc_condition == "BULLISH": buy_threshold = 80
        elif btc_condition == "SIDEWAYS": buy_threshold = 85
        else: buy_threshold = 90
...
def check_and_send_weekly_recap():
    ...
    if now.weekday() == 6 and now.hour == 23:
        ...
        message += f"━━━━━━━━━━━━━━━━━━━━\n"
        message += f"🤖 Bot V2.1 berjalan dengan baik!"
...
# ==========================================
def main():
    print(f"🕒 Bot V2.1 dimulai: {datetime.now(UTC7).strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
    ...
    for pair in pairs:
        ...
        # ==========================================
            ...
            if signal in ["BUY", "BUY_STRONG"]:
                ...
                send_telegram_alert(signal, pair, current_price, f"SL: ${sl_price:.4f} (ATR-based)", score=score, reasons=reasons)
                stats['BUY'] += 1
            elif signal == "WATCH":
