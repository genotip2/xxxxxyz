...
def generate_signal(pair):
    ...
    - Untuk sinyal BUY, informasi indikator (RSI, ADX, Stoch K) tidak disertakan dalam pesan detail.
    ...
    # Untuk sinyal BUY, hilangkan informasi indikator dari pesan detail
...
def send_telegram_alert(signal_type, pair, current_price, details=""):
    ...
    - Informasi indikator (RSI, ADX, Stoch K) diambil langsung di fungsi ini dan ditampilkan.
    """
    ...
    # Hilangkan penyisipan 'details' dari pesan notifikasi
    # Tambahkan informasi indikator (RSI, ADX, Stoch K) ke dalam pesan
    try:
        entry_analysis = analyze_pair_interval(pair, TIMEFRAME_ENTRY)
        if entry_analysis:
