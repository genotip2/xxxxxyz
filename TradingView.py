...
# Konfigurasi Cache Pair:
TOP_PAIR_TO_CACHED = 200
CACHE_FILE = 'top_pairs_cache.json'
...
#############################################
#  FUNGSI CACHE PAIR DARI BINANCE (CoinGecko) #
#############################################

def update_top_pairs_cache(current_month):
    """
    Ambil semua halaman tickers dari Binance melalui CoinGecko, 
    filter pair dengan target USDT, urutkan berdasarkan volume USD secara menurun, 
    ambil top pair sesuai konfigurasi TOP_PAIR_TO_CACHED, dan simpan ke file cache.
    """
    ...
    usdt_pairs = [t for t in all_tickers if t.get('target') == 'USDT']
    # Urutkan berdasarkan volume USD secara menurun
    sorted_pairs = sorted(usdt_pairs, key=lambda x: x.get('converted_volume', {}).get('usd', 0), reverse=True)
    # Ambil top pair sesuai konfigurasi
    top_pairs = [f"{p.get('base')}USDT" for p in sorted_pairs[:TOP_PAIR_TO_CACHED]]

    # Simpan ke file cache beserta info bulan terakhir update
    cache_data = {
    ...
    try:
        ...
        print("✅ Cache pair diperbarui.")
    except Exception as e:
        print(f"❌ Gagal menyimpan cache pair: {e}")
    return top_pairs

def get_binance_top_pairs():
    """
    Ambil pair dari file cache. Jika file cache tidak ada atau bulan cache tidak sesuai dengan bulan berjalan,
    perbarui cache dengan mengambil data dari semua halaman CoinGecko.
    """
    ...
    if os.path.exists(CACHE_FILE):
        try:
            ...
            if cache_data.get("last_updated") == current_month:
                print("ℹ️ Menggunakan cache pair dari file.")
                return cache_data.get("pairs", [])
            else:
                print("ℹ️ Cache sudah lama, memperbarui cache pair.")
        except Exception as e:
            print(f"❌ Gagal membaca cache file: {e}")
    # Perbarui cache jika file tidak ada atau sudah tidak valid
    return update_top_pairs_cache(current_month)
...
#############################################
# GENERATE SINYAL TRADING DENGAN SCORING & BEST ENTRY
#############################################
