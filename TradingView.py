...
# File cache untuk menyimpan daftar pair top 200
...
PAIR_TO_ANALYZE = 200         # (parameter lama, tidak lagi digunakan karena analisis diambil dari cache)
...
def update_pairs_cache():
    ...
    memfilter pair dengan target USDT, mengambil 200 pair teratas berdasarkan volume trading,
    lalu menyimpannya ke file cache dengan urutan dari volume terkecil ke terbesar.
    """
    ...
    # Urutkan berdasarkan volume trading USD secara menurun
    sorted_tickers = sorted(usdt_tickers, key=lambda x: x.get('converted_volume', {}).get('usd', 0), reverse=True)
    # Ambil 200 pair teratas
    top_200 = sorted_tickers[:200]
    # Urutkan ulang dari volume terkecil ke terbesar
    top_200_sorted = sorted(top_200, key=lambda x: x.get('converted_volume', {}).get('usd', 0))
    # Bentuk daftar pair dengan format "BASEUSDT"
    pairs_list = [f"{ticker.get('base')}USDT" for ticker in top_200_sorted]
...
def main():
    load_active_buys()
    # Gunakan daftar pair dari file cache
